  ...
  return (
    <div className={"flex flex-row gap-1 text-[var(--text-color)]"}>
      <input
        ...
        min={0}
        max={60}
        onChange={(e): void => { setNewTime([parseStrValue(e.target.value), time[1], time[2]]) }}
      ...
      <input
        ...
        min={0}
        max={60}
        onChange={(e): void => { setNewTime([time[0], parseStrValue(e.target.value), time[2]]) }}
        className={"w-14 pl-2 bg-[var(--secondary-bg-color)] rounded-lg"}
      />
      <p className={"text-right text-[var(--secondary-bg-color)]"}>:</p>
      ...
      <input
        ...
        min={0}
        max={60}
        onChange={(e): void => { setNewTime([time[0], time[1], parseStrValue(e.target.value)]) }}
        className={"w-14 pl-2 bg-[var(--secondary-bg-color)] rounded-lg"}
      />
    </div>
  )
